...
module.exports = function (file, template) {
  ...
  estraverse.traverse(syntax, {
    enter: function(node, parent) {
      if (node.type === 'CallExpression' && node.callee.type === 'Identifier' && node.callee.name === 'StateMachine' && node.arguments[0].type === 'ObjectExpression') {
        target = node;
      }
  ...
  for (var prop in fsm.callbacks) {
    var result = /(on)(entered|enter|leave)?([a-zA-Z0-9_]+)/.exec(prop),
        activity = result[0],
        stage = result[2],
        name = result[3];
